...
%     figure,imagesc(wrap(angle(z)))
    ...
    lmb_max2   = norm(angle(z), inf);
     ...
     [temp2]    = admm_tv(tem(:), tau1*lmb_max1, Params.rho, [Nob Nob],10);
      x1m(intervalo,intervalo)   =  reshape(temp2,[Nob Nob]);
     %phase 
     x1f        =zeros(N1);
     temp       =puma_ho(angle(z(intervalo,intervalo)),.5,'verbose','no');    
     [temp2]    = admm_tv(temp(:), tau2*lmb_max2, Params.rho, [Nob Nob],10); 
     x1f(intervalo,intervalo) = reshape(temp2,[Nob Nob]);
    x1=x1m.*exp(1i.*x1f);
